...
const startStr = '<div id="substack-feed" class="articles-grid">';
const start = html.indexOf(startStr);

// Find the footer tag which signals the end of the sections we want to replace
const endStr = '<footer class="site-footer">';
const end = html.indexOf(endStr, start);

if (start !== -1 && end !== -1) {
  // We go back slightly to include the opening of the section if needed, but since we lost the Substack wrapper entirely in the bad replace, we will just rebuild from the substack-feed div downwards.
  
  // Let's find the start of the articles section
  const sectionStartStr = '<section class="articles-section fade-in" id="artigos">';
  const sectionStart = html.indexOf(sectionStartStr);
  
  const replacement = `<section class="articles-section fade-in" id="artigos">
    <div class="container">
...
----------------------------------------------------------------------------------------------------- -->
  ...
  if (sectionStart !== -1) {
    html = html.substring(0, sectionStart) + replacement + html.substring(end);
    fs.writeFileSync('index.html', html, 'utf8');
    console.log('Successfully rebuilt the bottom of the page.');
  } else {
